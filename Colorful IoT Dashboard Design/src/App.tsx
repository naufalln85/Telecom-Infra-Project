...
function ProjectModal({ onClose }: { onClose: () => void }) {
  ...
  const [projects, setProjects] = React.useState(['Smart Home v2','Project Alya'])
  const [active, setActive] = React.useState('Smart Home v2')
  const create = () => {
    if (input.trim() && !projects.includes(input.trim())) {
      setProjects(p => [...p, input.trim()]); setInput('')
    }
  }
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:300 }} onClick={onClose}>
      <div style={{ background: C.surface, border:`1px solid ${C.border}`, borderRadius:20, padding:28, width:460, maxWidth:'92vw', boxShadow:'0 24px 64px rgba(0,0,0,0.6)' }} onClick={e=>e.stopPropagation()}>
        ...
        {projects.map((p) => (
          <div key={p} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'13px 16px', borderRadius:10, marginBottom:8, background: active===p ? `${C.coral}15` : C.surface2, border:`1px solid ${active===p ? C.coral+'44' : C.border}`, cursor:'pointer', transition:'all .15s' }} onClick={()=>setActive(p)}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <Icon name="layers" size={14} color={active===p?C.coral:C.muted} />
              <span style={{ fontWeight:600, fontSize:14, color: C.light }}>{p}</span>
              {active===p && <Pill text="ACTIVE" color={C.coral} />}
            </div>
            {projects.length > 1 && (
              <button onClick={e=>{e.stopPropagation();setProjects(prev=>prev.filter(x=>x!==p));if(active===p)setActive(projects.find(x=>x!==p)??'')}} style={{ background:'transparent', border:'none', color: C.muted, cursor:'pointer', padding:4 }}>
        ...
        {sidebar && (
          <button onClick={()=>setShowProject(true)} style={{ margin:'10px 10px 4px', padding:'10px 12px', borderRadius:10, background:`linear-gradient(135deg,${C.coral}18,${C.purple}18)`, border:`1px solid ${C.coral}33`, cursor:'pointer', textAlign:'left', transition:'border-color .15s', fontFamily:'Outfit,sans-serif' }}
            ...
            <div style={{ fontSize:12, color: C.light, fontWeight:700, margin:'3px 0', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              Smart Home v2 <Icon name="chevron" size={12} color={C.muted} />
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:4 }}>
              <StatusDot color={C.coral} />
              <span style={{ fontSize:9, color: C.muted }}>3/6 devices online</span>
            </div>
          </button>
        )}
        ...
        <header style={{ height:62, display:'flex', alignItems:'center', gap:16, padding:'0 24px', borderBottom:`1px solid ${C.border}`, background: C.surface, flexShrink:0 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            <div>
              ...
              <div style={{ fontSize:10, color: C.muted, lineHeight:1 }}>Smart Home v2</div>
            </div>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, paddingLeft:10, borderLeft:`1px solid ${C.border}`, cursor:'pointer' }} onClick={()=>setNav('settings')}>
              ...
              <div>
                <div style={{ fontWeight:700, fontSize:13, color: C.light, lineHeight:1.2 }}>Alex Ragnarsson</div>
                <div style={{ fontSize:10, color: C.muted, lineHeight:1 }}>Admin · Smart Home</div>
              </div>
            </div>
          </div>
        </header>
