...
export function LandingPage({ isDark, onToggleTheme, onLogin, onRegister }: { isDark: boolean; onToggleTheme: () => void; onEnter: () => void; onLogin: AuthHandler; onRegister?: AuthHandler }) {
  ...
  return <div style={{ minHeight:"100vh", color:"var(--c-text)", background:"var(--c-bg)", fontFamily:"Outfit,sans-serif", overflowX:"hidden" }}>
    ...
    <nav className="landing-nav" style={{ position:"sticky", top:0, zIndex:100, height:64, display:"flex", alignItems:"center", padding:"0 clamp(16px,5vw,40px)", background:"var(--c-surface)", borderBottom:"1px solid var(--c-border)" }}>
      ...
      <div className="landing-links" style={{ display:"flex", gap:24, marginLeft:40 }}>
      ...
      <div className="landing-actions" style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:10 }}><button onClick={onToggleTheme} className="landing-btn-theme" style={{ border:"1px solid var(--c-border)", borderRadius:8, padding:"7px 10px", background:"transparent", color:"var(--c-muted)", cursor:"pointer" }}>{isDark ? "Light" : "Dark"}</button><button onClick={openAuth} style={{ border:"1px solid var(--c-border)", borderRadius:10, padding:"8px 16px", background:"transparent", color:"var(--c-text)", cursor:"pointer", fontWeight:600 }}>Sign In</button><button onClick={openAuth} className="landing-btn-primary" style={{ border:0, borderRadius:10, padding:"8px 16px", background:`linear-gradient(135deg,${C.coral},${C.purple})`, color:"#fff", cursor:"pointer", fontWeight:700 }}>Enter Console →</button></div>
    </nav>
    <main style={{ maxWidth:1100, margin:"auto", padding:"0 16px" }}>
      <section style={{ textAlign:"center", padding:"60px 16px 40px" }}><div style={{ display:"inline-flex", color:C.coral, background:`${C.coral}15`, border:`1px solid ${C.coral}33`, borderRadius:24, padding:"6px 18px", fontSize:11, fontWeight:700, letterSpacing:".08em" }}>⚡ LOW-CODE IOT PLATFORM · ENTERPRISE READY</div><h1 style={{ fontSize:"clamp(28px,7vw,60px)", lineHeight:1.1, margin:"24px 0 12px" }}>Connect, Monitor & Control</h1><h1 style={{ fontSize:"clamp(28px,7vw,60px)", lineHeight:1.1, margin:"0 0 20px", background:`linear-gradient(90deg,${C.coral},${C.magenta},${C.purple})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Any IoT Device at Scale</h1><p style={{ maxWidth:560, margin:"0 auto 28px", color:"var(--c-muted)", lineHeight:1.7, fontSize:14 }}>Yugma adalah workspace IoT terpadu untuk menghubungkan perangkat, memvisualkan data, mengatur automasi, dan membangun insight dari telemetry.</p><button onClick={openAuth} style={{ padding:"14px 30px", border:0, borderRadius:12, background:`linear-gradient(135deg,${C.coral},${C.purple})`, color:"#fff", fontWeight:700, fontSize:15, cursor:"pointer", boxShadow:`0 8px 32px ${C.coral}44` }}>Get Started Free →</button></section>
      <section className="landing-grid-4" style={{ display:"grid", gridTemplateColumns:"repeat(4, minmax(0, 1fr))", marginBottom:48, border:"1px solid var(--c-border)", borderRadius:16, overflow:"hidden" }}>{[['10M+','Devices Connected'],['3','Protocols Supported'],['99.99%','Uptime SLA'],['< 10ms','Alert Latency']].map(([value,label]) => <div key={label} style={{ padding:"20px 10px", textAlign:"center", background:"var(--c-surface)", borderRight:"1px solid var(--c-border)" }}><b style={{ fontSize:22 }}>{value}</b><div style={{ fontSize:11, color:"var(--c-muted)", marginTop:4 }}>{label}</div></div>)}</section>
      <section id="features" style={{ marginBottom:50 }}><header style={{ textAlign:"center", marginBottom:28 }}><div style={{ color:C.coral, fontSize:11, fontWeight:700, letterSpacing:".1em" }}>PLATFORM CAPABILITIES</div><h2 style={{ fontSize:24, margin:"8px 0" }}>Everything you need, nothing you don't</h2></header><div className="landing-grid-3" style={{ display:"grid", gridTemplateColumns:"repeat(3, minmax(0, 1fr))", gap:16 }}>{features.map(feature => <article key={feature.title} style={{ padding:20, background:"var(--c-surface)", border:"1px solid var(--c-border)", borderRadius:16 }}><div style={{ width:40, height:40, display:"grid", placeItems:"center", borderRadius:12, background:`${feature.color}18`, marginBottom:14 }}><Icon name={feature.icon} size={20} color={feature.color} /></div><b>{feature.title}</b><p style={{ color:"var(--c-muted)", fontSize:12, lineHeight:1.6 }}>{feature.desc}</p></article>)}</div></section>
      <section id="docs" style={{ marginBottom:50, textAlign:"center" }}><div style={{ color:C.purple, fontSize:11, fontWeight:700, letterSpacing:".1em" }}>QUICK START</div><h2 style={{ fontSize:24, margin:"8px 0 24px" }}>Up and running in under 5 minutes</h2><div className="landing-grid-4" style={{ display:"grid", gridTemplateColumns:"repeat(4, minmax(0, 1fr))", gap:14 }}>{[['01','Create a Project'],['02','Connect Devices'],['03','Build Your Canvas'],['04','Monitor & Automate']].map(([number,title]) => <div key={number} style={{ padding:"20px 12px", background:"var(--c-surface)", border:"1px solid var(--c-border)", borderRadius:14 }}><div style={{ width:36, height:36, borderRadius:"50%", display:"grid", placeItems:"center", margin:"0 auto 10px", color:"#fff", fontWeight:800, fontSize:13, background:`linear-gradient(135deg,${C.coral},${C.purple})` }}>{number}</div><b style={{ fontSize:13 }}>{title}</b></div>)}</div></section>
      <section id="cta" style={{ textAlign:"center", padding:"44px 20px", marginBottom:48, borderRadius:20, background:`linear-gradient(135deg,${C.coral}18,${C.purple}18)`, border:`1px solid ${C.coral}33` }}><h2 style={{ fontSize:24, margin:"0 0 10px" }}>Ready to connect your first device?</h2><p style={{ color:"var(--c-muted)", margin:"0 0 24px", fontSize:13 }}>Mulai membangun workspace IoT Anda sekarang.</p><button onClick={openAuth} style={{ padding:"14px 28px", border:0, borderRadius:12, background:`linear-gradient(135deg,${C.coral},${C.purple})`, color:"#fff", cursor:"pointer", fontWeight:700 }}>Start Building Now →</button></section>
    </main>
  </div>
}
